...
class KAPService:
    ...
    # Önemli anahtar kelimeler (sentiment için)
    POSITIVE_KEYWORDS = [
        "kar", "kâr", "artış", "büyüme", "rekor", "başarı", "anlaşma", "sözleşme",
        "yatırım", "ihale", "kazandı", "temettü", "dağıtım", "yüksek", "olumlu",
        "ödül", "sertifika", "onay", "lisans", "genişleme", "satış", "sipariş"
    ]
    
    NEGATIVE_KEYWORDS = [
        "zarar", "düşüş", "azalış", "iptal", "erteleme", "soruşturma", "ceza",
        "dava", "risk", "tehlike", "kapanış", "iflas", "borç", "olumsuz",
        "gerileme", "daralma", "kayıp", "istifa", "sona erdi"
    ]
    ...
    async def fetch_kap_news_for_symbol(self, symbol: str) -> List[Dict[str, Any]]:
        """
        Tek bir hisse için KAP haberlerini çek
        Google News'den KAP ile ilgili haberleri filtreler
        """
        # Şirket adını bul
        from .real_news_service import GoogleNewsService
        stock_name = GoogleNewsService.STOCK_NAMES.get(symbol, symbol)
        
        # KAP ve resmi duyuru araması
        search_queries = [
            f"{stock_name} KAP bildirimi",
            f"{stock_name} özel durum açıklaması",
            f"{symbol} hisse duyuru"
        ]
            ...
            try:
                ...
                for entry in feed.entries[:5]:
                    ...
                    if ' - ' in title:
                        ...
                        title = parts[0]
                        source = parts[1] if len(parts) > 1 else "Haber"
                    else:
                        source = "Haber"
                    ...
                    # Kategori ve sentiment
                    category = self._categorize_news(title)
                    sentiment = self._analyze_sentiment(title, summary)
                    importance = self.CATEGORY_IMPORTANCE.get(category, {}).get("importance", "low")
                    ...
                    news_item = {
                        ...
                        "category": category,
                        "category_name": self.CATEGORY_IMPORTANCE.get(category, {}).get("name", "Diğer"),
                ...
                print(f"KAP haber hatası ({symbol}): {e}")
                continue
...
class DailyNewsCollector:
    ...
    async def collect_news_for_batch(self, symbols: List[str], batch_size: int = 10) -> Dict[str, int]:
        """Grup halinde hisse haberleri topla"""
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            for symbol in batch:
                try:
                    # Haberleri çek
                    news = await self.kap_service.fetch_kap_news_for_symbol(symbol)
                    
                    if news:
                        # Veritabanına kaydet
                        saved = self.kap_service.save_news_to_db(news)
                        results["total_news"] += saved
                        results["success"] += 1
                    else:
                        results["success"] += 1  # Haber yok ama hata da yok
                        
                except Exception as e:
                    print(f"Haber toplama hatası ({symbol}): {e}")
                    results["error"] += 1
            
            # Batch arası bekleme (rate limiting)
            await asyncio.sleep(1)
            print(f"Batch {i//batch_size + 1} tamamlandı: {len(batch)} hisse işlendi")
