...
export default function AdvancedFilterPanel({ 
  ...
  useEffect(() => {
    // Sektörleri yükle
    fetch('http://localhost:8000/api/filters/sectors')
      .then(res => res.json())
      .then(data => {
        if (data.success) {
      ...
      .catch(err => console.error('Sektörler yüklenemedi:', err));
  }, []);
                ...
                {INDICES.map(index => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm transition ${
                      filters.indices.includes(index)
